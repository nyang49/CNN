...
close all;
clear all;
comp='D:/UW-MADISON/';%the computer user location: C:\Users\nyang\Desktop\CNN\Generated_Mix
workdir=[comp 'CNN/Generated_Mix'];%%the working folder
%libdir=[comp 'Desktop/gissmo_lib/'];
cd(workdir);
rng(1);%%random seed
...
%%sample control
args.nsample=10;%%number of samples for each group
...
%%% spectra simulation
load(['wholegissmo_spectral.complex.more.mat']);
load(['wholegissmo_spectral.real.list.mat']);
load(['gissmo_tab.mat']);
spec_args=struct();
spec_args.ppm=ppm;
...
writetable(result,'result10.csv','Delimiter',',');
